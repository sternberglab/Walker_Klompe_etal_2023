...
#----------------------------------
"""Determine extent to which barcodes are correctly associated with transposon end seqences, based on MiSeq run"""
#----------------------------------
...
def main():
  ...
  library_info_dict = {
    "MiSeq-right" : {
      "dataset" : "210625_Right_pDNA_MiSeq",
      "end_length" : 127,
      "output_dir" : "6_uncoupling/right/"},
    "MiSeq-left" : {
      "dataset" : "210625_Left_pDNA_MiSeq",
      "end_length" : 152,
      "output_dir" : "6_uncoupling/left/"}
  }

  for library in library_info_dict:
    os.makedirs(library_info_dict[library]['output_dir'], exist_ok=True)
    ...
    def count_matching_sequences():
      ...
      with open(output_dir, 'w') as out:
        ...
        writer.writerow(["barcode", "total reads", "# matches", "# mismatches", "average nt-difference", "% matches", "# of unique mismatched seqs", "most common", "% most abundant incorrect seq"])
        for key in barcode_matches_seq_counter:
          ...
          try: percent_max_incorrect = (most_common / total_reads)*100
          except: percent_max_incorrect = "NA"
          writer.writerow([key, total_reads, matches, mismatches, average_number_mismatches, percent_matches, len(set(mismatched_sequences[key])), most_common, percent_max_incorrect])
