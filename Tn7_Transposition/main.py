...
def main():
  ...
  for dataset in datasets: 
    #analyze()
    input_dir = info[dataset]['output_dir'] + "counts.csv"
    df = pd.read_csv(input_dir)
    enrichment_list.append([dataset, df["abundance"][0], 100-df["abundance"][0]])

  with open('2_counts/summary.csv','wt') as out:
    writer  = csv.writer(out)
    # for i in range(0,8):
    #   row = [item[i] for item in enrichment_list]
    #   writer.writerow(row)

    writer.writerow(["sample", "percent_unintegrated", "percent_integrated"])
    for i in enrichment_list:
      writer.writerow(i)

  
  # item_length = len(enrichment_list[0])
  # with open('test.csv', 'w') as test_file:
  #   file_writer = csv.writer(test_file)
  #   for i in range(item_length):
  #     print(enrichment_list[i])
      # file_writer.writerow([x[i] for x in enrichment_list])
    # def analyze():
    #   # create_info_csv()
    #   # count_reads()
